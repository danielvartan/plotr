...
First of all, thanks for considering contributing to `plotr`! 👍 It’s
people like you that make it rewarding for us—the project maintainers—to
work on `plotr`. 😊

`plotr` is an open source project, maintained by people who care. We are
...
Think `plotr` is useful? Help others discover it by sharing in person,
on social media, or through a blog post. Don’t forget to **star** the
[GitHub repository](https://github.com/danielvartan/plotr/)!

Using `plotr` for a paper you are writing? Consider [citing
it](https://danielvartan.github.io/plotr/authors.html).

### Propose an Idea 💡

Have an idea for a new `plotr` feature? Take a look at the
[documentation](https://danielvartan.github.io/plotr/) and [issue
list](https://github.com/danielvartan/plotr/issues) to see if it isn’t
included or suggested yet. If not, suggest your idea as an [issue on
GitHub](https://github.com/danielvartan/plotr/issues). While we can’t
...
Using `plotr` and discovered a bug? That’s annoying! Don’t let others
have the same experience and report it as an [issue on
GitHub](https://github.com/danielvartan/plotr/issues/new) so we can fix
it. A good bug report makes it easier for us to do so, so please
include:

- The content of
  [`utils::sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html)
  (Tip: use the [reprex](https://reprex.tidyverse.org/) package)
...
[This website](https://danielvartan.github.io/plotr/) is generated with
...
1.  Go to the `R` directory in the [code
    repository](https://github.com/danielvartan/plotr/)
...
Care to fix bugs or implement new functionality for `plotr`? Awesome! 👏
Have a look at the [issue
list](https://github.com/danielvartan/plotr/issues) and leave a comment
...
1.  Fork [this repository](https://github.com/danielvartan/plotr/) and
...
3.  Open the project in RStudio or other
...
Also note that this project use [Tidyverse design
